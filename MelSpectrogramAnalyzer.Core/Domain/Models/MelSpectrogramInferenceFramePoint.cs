...
public sealed class MelSpectrogramInferenceFramePoint
{
    public MelSpectrogramInferenceFramePoint(
        string name,
        int nowMs,
        int frameIndex,
        double[,]? linear,
    ...
    private static void ValidateMatrix(double[,]? matrix, bool allowNegativeInfinity)
    {
        ...
        {
            for (int mel = 0; mel < matrix.GetLength(1); mel++)
            {
                double value = matrix[ch, mel];
