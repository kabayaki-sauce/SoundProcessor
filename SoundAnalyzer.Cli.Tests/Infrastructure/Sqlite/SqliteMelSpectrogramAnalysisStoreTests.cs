...
public sealed class SqliteMelSpectrogramAnalysisStoreTests
{
    [Fact]
    public void Initialize_ShouldCreateNormalizedTableWithMsAnchor_WhenAnchorColumnIsMs()
    {
        ...
        try
        {
            using (SqliteMelSpectrogramAnalysisStore store = new(
                       dbFilePath,
                       "t_mel_spectrogram",
                       "ms",
            ...
            Assert.True(TableExists(dbFilePath, "t_mel_spectrogram"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "audio_name"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "window_size"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "ms"));
            Assert.False(ColumnExists(dbFilePath, "t_mel_spectrogram", "sample"));
            Assert.False(ColumnExists(dbFilePath, "t_mel_spectrogram", "name"));
            Assert.False(ColumnExists(dbFilePath, "t_mel_spectrogram", "window"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "bin_no"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "db"));
    ...
    public void Initialize_ShouldCreateNormalizedTableWithSampleAnchor_WhenAnchorColumnIsSample()
    {
        ...
        try
        {
            ...
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "audio_name"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "window_size"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "sample"));
            Assert.False(ColumnExists(dbFilePath, "t_mel_spectrogram", "ms"));
            Assert.False(ColumnExists(dbFilePath, "t_mel_spectrogram", "name"));
            Assert.False(ColumnExists(dbFilePath, "t_mel_spectrogram", "window"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "bin_no"));
            Assert.True(ColumnExists(dbFilePath, "t_mel_spectrogram", "db"));
    ...
    public void Initialize_ShouldFail_WhenExistingTableBinCountDiffers_WithExistingRows()
    {
        ...
        try
        {
            using (SqliteMelSpectrogramAnalysisStore first = new(
                       dbFilePath,
                       "t_mel_spectrogram",
                       "ms",
                       SqliteConflictMode.Error,
            ...
            {
                first.Initialize();
                first.Write(new MelSpectrogramAnalysisPoint("Song", 0, 50, 10, CreateBins(8, -12)));
                first.Complete();
            }

            using SqliteMelSpectrogramAnalysisStore second = new(
                dbFilePath,
                "t_mel_spectrogram",
                "ms",
                SqliteConflictMode.Error,
            ...
            Assert.Equal(CliErrorCode.MelTableBinCountMismatch, exception.ErrorCode);
        }
        finally
        {
            DeleteIfExists(dbFilePath);
    ...
    public void Initialize_ShouldFail_WhenExistingAnchorColumnDiffers()
    {
        ...
        try
        {
            ...
            CliException exception = Assert.Throws<CliException>(() => second.Initialize());
            Assert.Equal(CliErrorCode.MelTableSchemaMismatch, exception.ErrorCode);
        }
        finally
        {
            DeleteIfExists(dbFilePath);
    ...
    public void Initialize_ShouldFail_WhenLegacyWideSchemaDetected()
    {
        ...
        try
        {
            CreateLegacyWideMelSpectrogramTable(dbFilePath);
    ...
    public void Write_ShouldUpsertAndKeepCreateAt_ForMsAnchor()
    {
        ...
        try
        {
            MelSpectrogramAnalysisPoint firstPoint = new("Song", 0, 50, 10, CreateBins(4, -10));
            ...
            MelSpectrogramAnalysisPoint updatedPoint = new("Song", 0, 50, 10, CreateBins(4, -3));
            using (SqliteMelSpectrogramAnalysisStore store = new(
                       dbFilePath,
                       "t_mel_spectrogram",
                       "ms",
    ...
    public void Write_ShouldSkipDuplicate_WhenSkipDuplicateIsEnabled_ForSampleAnchor()
    {
        ...
        try
        {
            MelSpectrogramAnalysisPoint point = new("Song", 0, 2048, 512, CreateBins(4, -10));
    ...
    public void Write_ShouldSupportHighBinCount_Over2000()
    {
        ...
        try
        {
            MelSpectrogramAnalysisPoint point = new("Song", 0, 4096, 1024, CreateBins(binCount, -6));
    ...
    public void Complete_ShouldFailOnDuplicateRows_WhenNewTableUsesDeferredUniqueIndex()
    {
        ...
        try
        {
            MelSpectrogramAnalysisPoint point = new("Song", 0, 50, 10, CreateBins(4, -10));
    ...
    private static void CreateLegacyWideMelSpectrogramTable(string dbFilePath)
    {
        using SqliteConnection connection = OpenConnection(dbFilePath);
        using SqliteCommand command = connection.CreateCommand();
        command.CommandText =
            $"""
            CREATE TABLE "t_mel_spectrogram" (
    ...
    private static int CountLegacyWideBinColumns(string dbFilePath)
    {
        using SqliteConnection connection = OpenConnection(dbFilePath);
        using SqliteCommand command = connection.CreateCommand();
        command.CommandText = "PRAGMA table_info(\"t_mel_spectrogram\");";
...
#pragma warning disable CA2100
        ...
        int binNo)
    {
        ...
        command.CommandText = anchorColumnName.Equals("sample", StringComparison.OrdinalIgnoreCase)
            ? "SELECT create_at, modified_at, db FROM \"t_mel_spectrogram\" WHERE audio_name = 'Song' AND ch = 0 AND window_size = 50 AND sample = $anchor AND bin_no = $bin_no;"
            : "SELECT create_at, modified_at, db FROM \"t_mel_spectrogram\" WHERE audio_name = 'Song' AND ch = 0 AND window_size = 50 AND ms = $anchor AND bin_no = $bin_no;";
...
#pragma warning restore CA2100
    ...
    private static int ReadMaxBinNo(string dbFilePath)
    {
        using SqliteConnection connection = OpenConnection(dbFilePath);
        using SqliteCommand command = connection.CreateCommand();
        command.CommandText = "SELECT COALESCE(MAX(bin_no), 0) FROM \"t_mel_spectrogram\";";
        object? scalar = command.ExecuteScalar();
    ...
    private static long ReadRowCount(string dbFilePath)
    {
        using SqliteConnection connection = OpenConnection(dbFilePath);
        using SqliteCommand command = connection.CreateCommand();
        command.CommandText = "SELECT COUNT(1) FROM \"t_mel_spectrogram\";";
        object? scalar = command.ExecuteScalar();
        return scalar is long count ? count : 0;
    }
    ...
    {
        string directoryPath = Path.Combine(Path.GetTempPath(), $"sound-analyzer-mel-db-tests-{Guid.NewGuid():N}");
