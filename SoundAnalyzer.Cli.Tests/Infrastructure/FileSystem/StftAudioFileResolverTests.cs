...
public sealed class StftAudioFileResolverTests
{
    [Fact]
    public void Resolve_ShouldScanOnlyTopDirectoryFiles_WhenRecursiveIsFalse()
    {
        ...
        try
        {
            ...
            ResolvedStftAudioFiles resolved = StftAudioFileResolver.Resolve(root, recursive: false);
    ...
    public void Resolve_ShouldScanRecursively_WhenRecursiveIsTrue()
    {
        ...
        try
        {
            ...
            ResolvedStftAudioFiles resolved = StftAudioFileResolver.Resolve(root, recursive: true);
    ...
    public void Resolve_ShouldFail_WhenDuplicateNamesExistAcrossInputSet_CaseInsensitive()
    {
        ...
        try
        {
            ...
            CliException exception = Assert.Throws<CliException>(
                () => StftAudioFileResolver.Resolve(root, recursive: true));

            Assert.Equal(CliErrorCode.DuplicateStftAnalysisName, exception.ErrorCode);
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"sound-analyzer-stft-resolver-tests-{Guid.NewGuid():N}");
