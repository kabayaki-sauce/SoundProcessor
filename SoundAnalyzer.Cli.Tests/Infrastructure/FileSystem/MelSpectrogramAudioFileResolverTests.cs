...
public sealed class MelSpectrogramAudioFileResolverTests
{
    [Fact]
    public void Resolve_ShouldScanOnlyTopDirectoryFiles_WhenRecursiveIsFalse()
    {
        ...
        try
        {
            ...
            ResolvedMelSpectrogramAudioFiles resolved = MelSpectrogramAudioFileResolver.Resolve(root, recursive: false);
    ...
    public void Resolve_ShouldScanRecursively_WhenRecursiveIsTrue()
    {
        ...
        try
        {
            ...
            ResolvedMelSpectrogramAudioFiles resolved = MelSpectrogramAudioFileResolver.Resolve(root, recursive: true);
    ...
    public void Resolve_ShouldFail_WhenDuplicateNamesExistAcrossInputSet_CaseInsensitive()
    {
        ...
        try
        {
            ...
            CliException exception = Assert.Throws<CliException>(
                () => MelSpectrogramAudioFileResolver.Resolve(root, recursive: true));

            Assert.Equal(CliErrorCode.DuplicateMelAnalysisName, exception.ErrorCode);
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"sound-analyzer-mel-resolver-tests-{Guid.NewGuid():N}");
