...
internal static class MelSpectrogramAudioFileResolver
{
    ...
    public static ResolvedMelSpectrogramAudioFiles Resolve(string inputDirectoryPath, bool recursive)
    {
        ...
        List<MelSpectrogramAudioFile> files = SelectPreferredCandidates(paths, inputDirectoryPath);
        ...
        return new ResolvedMelSpectrogramAudioFiles(files, directoryCount);
    }

    private static List<MelSpectrogramAudioFile> SelectPreferredCandidates(string[] paths, string inputDirectoryPath)
    {
        ...
        List<MelSpectrogramAudioFile> resolved = new(orderedKeys.Count);
        for (int i = 0; i < orderedKeys.Count; i++)
        {
            string key = orderedKeys[i];
            FileCandidate selected = SelectPreferred(grouped[key]);
            resolved.Add(new MelSpectrogramAudioFile(selected.Name, selected.Path));
        }

        return resolved;
    }

    private static void EnsureNoDuplicateNames(List<MelSpectrogramAudioFile> files)
    {
        ...
        {
            MelSpectrogramAudioFile file = files[i];
            if (!names.Add(file.Name))
            {
                throw new CliException(CliErrorCode.DuplicateMelAnalysisName, file.Name);
            }
        }
    }
