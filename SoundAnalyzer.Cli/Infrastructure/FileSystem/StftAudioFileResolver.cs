...
internal static class StftAudioFileResolver
{
    ...
    public static ResolvedStftAudioFiles Resolve(string inputDirectoryPath, bool recursive)
    {
        ...
        List<StftAudioFile> files = SelectPreferredCandidates(paths, inputDirectoryPath);
        ...
        return new ResolvedStftAudioFiles(files, directoryCount);
    }

    private static List<StftAudioFile> SelectPreferredCandidates(string[] paths, string inputDirectoryPath)
    {
        ...
        List<StftAudioFile> resolved = new(orderedKeys.Count);
        for (int i = 0; i < orderedKeys.Count; i++)
        {
            string key = orderedKeys[i];
            FileCandidate selected = SelectPreferred(grouped[key]);
            resolved.Add(new StftAudioFile(selected.Name, selected.Path));
        }

        return resolved;
    }

    private static void EnsureNoDuplicateNames(List<StftAudioFile> files)
    {
        ...
        {
            StftAudioFile file = files[i];
            if (!names.Add(file.Name))
            {
                throw new CliException(CliErrorCode.DuplicateStftAnalysisName, file.Name);
            }
        }
    }
