...
public sealed class StftInferenceNowTensorPoint
{
    public StftInferenceNowTensorPoint(
        string name,
        int nowMs,
        double[,,]? linear,
        double[,,]? db)
    ...
    private static void ValidateTensor(double[,,]? tensor, bool allowNegativeInfinity)
    {
        ...
        {
            for (int freq = 0; freq < tensor.GetLength(1); freq++)
            {
                for (int frame = 0; frame < tensor.GetLength(2); frame++)
                {
                    double value = tensor[ch, freq, frame];
